...
namespace Utils
{
	namespace DirectoryManagement
	{
		...
		class RecursiveRmdirHelper
		{
		public:
			DirFileList path;
			size_t index;
			RecursiveRmdirHelper(const char* _path) : path(_path), index(0) {}
			RecursiveRmdirHelper(const std::string& _path) : path(_path), index(0) {}
			RecursiveRmdirHelper(RecursiveRmdirHelper&& other)
			{
		...
		int PathIsDirectory(const char* path);
